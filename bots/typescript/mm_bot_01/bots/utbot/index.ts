...
import { createZoClient, type ZoClient, ZoOrderbookStream } from "../../../../../exchanges/typescript/zerone.js";
import { log } from "../../../core/utils/logger.js";
import type { UTBotConfig } from "./config.js";
import { DonchianIndicator, type DonchianState } from "./indicator.js";
import {
  calcRiskBasedSize,
  getBalance,
  getMaxLeverage,
...
export type { UTBotConfig } from "./config.js";
...
export class UTBot {
  ...
  private indicator: DonchianIndicator | null = null;
  ...
  constructor(
    private readonly config: UTBotConfig,
  ...
  async run(): Promise<void> {
    ...
    log.info(`Donchian Bot is LIVE. Trading on ${this.config.candleMinutes}m candles...`);
  ...
  private async initialize(): Promise<void> {
    ...
    this.indicator = new DonchianIndicator(
      cfg.entryPeriod, cfg.minAdx, cfg.atrSlPeriod, cfg.adxPeriod,
    );
    ...
    log.config({
      Strategy: "Donchian Channel Breakout + ADX Filter",
      Market: this.marketSymbol,
      "Binance feed": `${binanceSymbol.toUpperCase()} 1m → ${cfg.candleMinutes}m`,
      "Channel": `${cfg.entryPeriod}-period high/low breakout`,
      "ADX filter": `ADX(${cfg.adxPeriod}) >= ${cfg.minAdx}`,
      ...
      "Break-even": "fee-based (covers round-trip fees)",
  ...
  private async warmUp(): Promise<void> {
    ...
    if (serverPos.size > 0) {
      log.warn(
        ...
        "Bot cannot track pre-existing entry — exits via time only.",
      );
    } else {
  ...
  private async handleCandleClose(candle: Candle): Promise<void> {
    ...
    if (!this.activePosition) {
      if (state.longSignal) {
        this.signalCount++;
        log.info(
          `>>> SIGNAL #${this.signalCount}: LONG BREAKOUT @ $${state.price.toFixed(2)} > ` +
          `channel high $${state.channelHigh.toFixed(2)} | ADX=${state.adx.toFixed(1)}`,
        );
        await this.openPosition("long", state.price, state.atr);
      } else if (state.shortSignal) {
        this.signalCount++;
        log.info(
          `>>> SIGNAL #${this.signalCount}: SHORT BREAKOUT @ $${state.price.toFixed(2)} < ` +
          `channel low $${state.channelLow.toFixed(2)} | ADX=${state.adx.toFixed(1)}`,
        );
  ...
  private async executeStopOrTP(reason: string, triggerPrice: number, isWin: boolean): Promise<void> {
    ...
    try {
      ...
      log.info(
        `[${reason}] Closed ${pos.side.toUpperCase()} | ` +
  ...
  // ── Close position (candle-based exits) ──────────────────────────────────
  ...
  private logCandleState(state: DonchianState): void {
    ...
    log.info(
      ...
      `CH[${state.channelLow.toFixed(2)}|${state.channelHigh.toFixed(2)}] | ` +
      `ADX=${state.adx.toFixed(1)} | ATR=${state.atr.toFixed(2)} | pos=${posStr}`,
    );
  }
  ...
  private async shutdown(): Promise<void> {
    log.info("Shutting down Donchian Bot...");
  ...
  private printBanner(): void {
    log.info(`
╔══════════════════════════════════════════════════════════╗
║    01 EXCHANGE — DONCHIAN BREAKOUT BOT                   ║
║    Strategy: 50-period Channel Breakout + ADX Filter     ║
║    Timeframe: 1h | Risk: 1% | RR: 1:3                   ║
╚══════════════════════════════════════════════════════════╝`);
  }
}
