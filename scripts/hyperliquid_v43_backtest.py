#!/usr/bin/env python3
"""v43 Hyperliquid BTC Strategy — Strict Validation
Enhancements over v40:
  1. ADX(14) > 20 filter → avoids sideways chop that bleeds funding
  2. Funding-rate adverse filter → skip entry when 1h funding > 0.01% (against)
     AND/OR 8h avg funding > 0.005% (against)
  3. Trailing stop → activates after price moves 1×ATR in profit; trails at 3×ATR;
     never resets tighter
  4. Partial profit target → close 50% at 3:1 R:R (entry ± 9×ATR_at_entry)
  5. Max holding time → force close after 30 bars (120 hours at 4h)
"""
...
def build_features_v43(df_4h: pd.DataFrame, df_daily: pd.DataFrame, funding: pd.DataFrame, warmup_daily_start: str = "2021-01-01") -> pd.DataFrame:
    out = df_4h.copy()
...
class ScenarioConfig:
    ...
    warmup_bars: int = 220
    profit_target_rr: float = 3.0         # 3:1 R:R → target = entry ± (stop_dist × 3)
    trail_activation_atr_mult: float = 1.0  # activate trail after 1x ATR in profit
    max_hold_bars: int = 30               # force close after 30 bars (120h)
    ...
    def run(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, Dict[str, float]]:
        ...
        for i in range(s.warmup_bars, len(df) - 1):
            ...
            if position == 1:  # long
                ...
                highest_close = max(highest_close, float(b["close"]))
                if not trail_activated and (highest_close - entry) >= s.trail_activation_atr_mult * atr_at_entry:
                    trail_activated = True

                if trail_activated:
                    new_trail_stop = highest_close - s.stop_atr_mult * float(b["atr"])
                    stop = max(stop, new_trail_stop)  # never tighten (move down)
                ...
                if stop_hit:
                    ...
                    trades.append({
                        ...
                        "bars_held": bars_held,
                        "reason": "trailing_stop" if trail_activated else "stop",
                    })
                    position, qty, entry, entry_time, signal_time, stop = 0, 0.0, 0.0, None, None, np.nan
                    highest_close, lowest_close = 0.0, float("inf")
                    trail_activated, profit_target_hit, bars_held = False, False, 0
                    atr_at_entry, profit_target = 0.0, 0.0
                    exited_this_bar = True

                # 2) intrabar profit target (partial close)
                if not exited_this_bar and not profit_target_hit and profit_target > 0 and b["high"] >= profit_target:
                    close_qty = qty * 0.5
                    exit_px = apply_slippage(profit_target, "long", "exit", s.exit_slippage_bps)
                    funding_pnl_half = funding_pnl_for_window(
                        self.funding, self.hourly_prices, entry_time, t, "long", close_qty,
                        s.funding_mode, s.funding_multiplier,
                    )
                    fees_half = self.trade_fee(close_qty, entry) + self.trade_fee(close_qty, exit_px)
                    pnl_half = close_qty * (exit_px - entry) - fees_half + funding_pnl_half
                    realized += pnl_half
                    funding_total += funding_pnl_half
                    fee_total += fees_half
                    trades.append({
                        "signal_time": signal_time,
                        "entry_time": entry_time,
                        ...
                        "exit": exit_px,
                        "qty": close_qty,
                        "gross_pnl": close_qty * (exit_px - entry),
                        "fees": -fees_half,
                        "funding": funding_pnl_half,
                        "pnl": pnl_half,
                        "hold_hours": (t - entry_time).total_seconds() / 3600,
                        "bars_held": bars_held,
                        "reason": "profit_target_50pct",
                    })
                    qty = qty * 0.5  # keep remaining
                    profit_target_hit = True

                # 3) max holding time
                if not exited_this_bar and bars_held >= s.max_hold_bars:
                ...
                # 4) signal exit
                if not exited_this_bar and bool(b["long_exit_signal"]):
                    exit_px = apply_slippage(float(next_bar["open"]), "long", "exit", s.exit_slippage_bps)
                    funding_pnl = funding_pnl_for_window(
                        self.funding, self.hourly_prices, entry_time, next_time, "long", qty,
                        s.funding_mode, s.funding_multiplier,
                ...
                # update trailing metrics
                lowest_close = min(lowest_close, float(b["close"]))
                if not trail_activated and (entry - lowest_close) >= s.trail_activation_atr_mult * atr_at_entry:
                    trail_activated = True

                if trail_activated:
                    new_trail_stop = lowest_close + s.stop_atr_mult * float(b["atr"])
                    stop = min(stop, new_trail_stop)  # never tighten (move up)
                ...
                # 2) intrabar profit target (partial close)
                if not exited_this_bar and not profit_target_hit and profit_target > 0 and b["low"] <= profit_target:
                    close_qty = qty * 0.5
                    exit_px = apply_slippage(profit_target, "short", "exit", s.exit_slippage_bps)
                    funding_pnl_half = funding_pnl_for_window(
                        self.funding, self.hourly_prices, entry_time, t, "short", close_qty,
                        s.funding_mode, s.funding_multiplier,
                    )
                    fees_half = self.trade_fee(close_qty, entry) + self.trade_fee(close_qty, exit_px)
                    pnl_half = close_qty * (entry - exit_px) - fees_half + funding_pnl_half
                    realized += pnl_half
                    funding_total += funding_pnl_half
                    fee_total += fees_half
                    trades.append({
                        ...
                        "exit": exit_px,
                        "qty": close_qty,
                        "gross_pnl": close_qty * (entry - exit_px),
                        "fees": -fees_half,
                        "funding": funding_pnl_half,
                        "pnl": pnl_half,
                        "hold_hours": (t - entry_time).total_seconds() / 3600,
                ...
                # 4) signal exit
                if not exited_this_bar and bool(b["short_exit_signal"]):
                    exit_px = apply_slippage(float(next_bar["open"]), "short", "exit", s.exit_slippage_bps)
                    funding_pnl = funding_pnl_for_window(
                        self.funding, self.hourly_prices, entry_time, next_time, "short", qty,
                        s.funding_mode, s.funding_multiplier,
            ...
            if position == 0:
                ...
                if bool(b["long_signal"]) and atr_dist > 0 and adx_ok and bool(b.get("funding_ok_long", True)):
                    raw_entry = float(next_bar["open"])
                    entry_px = apply_slippage(raw_entry, "long", "entry", s.entry_slippage_bps)
                    stop_px = entry_px - atr_dist
                    q = risk_usd / atr_dist
                    if q > 0:
                        position, qty, entry, entry_time, signal_time, stop = 1, float(q), float(entry_px), next_time, t, float(stop_px)
                        atr_at_entry = float(b["atr"])
                        profit_target = entry_px + s.profit_target_rr * (s.stop_atr_mult * atr_at_entry)
                        highest_close = float(entry_px)
                        lowest_close = float("inf")
                        trail_activated = False
                        profit_target_hit = False
                        bars_held = 0

                elif bool(b["short_signal"]) and atr_dist > 0 and adx_ok and bool(b.get("funding_ok_short", True)):
                    raw_entry = float(next_bar["open"])
            ...
            open_pnl = 0.0
            if position == 1:
                open_pnl = qty * (float(b["close"]) - entry)
            elif position == -1:
                open_pnl = qty * (entry - float(b["close"]))
        ...
        if position != 0 and entry_time is not None:
            ...
            funding_pnl = funding_pnl_for_window(
                self.funding, self.hourly_prices, entry_time, last_t, side, qty,
                s.funding_mode, s.funding_multiplier,
            )
            fees = self.trade_fee(qty, entry) + self.trade_fee(qty, exit_px)
            ...
            trades.append({
                ...
                "funding": funding_pnl,
                "pnl": pnl,
                "hold_hours": (last_t - entry_time).total_seconds() / 3600,
                "bars_held": bars_held,
                "reason": "eod",
            })
...
def trade_distribution(trades: pd.DataFrame) -> Dict[str, float]:
    if trades.empty:
        return {
            ...
            "avg_bars_held": 0.0, "median_bars_held": 0.0,
        }
    return {
        ...
        "avg_bars_held": round(float(trades["bars_held"].mean()), 2) if "bars_held" in trades.columns else 0.0,
        "median_bars_held": round(float(trades["bars_held"].median()), 2) if "bars_held" in trades.columns else 0.0,
    }
...
# ── load v40 results for comparison ──────────────────────────────────────────
...
def write_report(
    ...
    v40_comparison: Dict[str, object] | None,
    recommendation: str,
    verdict: str,
) -> None:
    ...
    if v40_comparison:
        ...
        v40_scenarios = v40_comparison.get("scenarios", [])
        v40_baseline = next((r for r in v40_scenarios if r.get("scenario") == "baseline_realistic"), None)
        v43_baseline = scenario_table[scenario_table["scenario"] == "baseline_realistic"].iloc[0] if len(scenario_table[scenario_table["scenario"] == "baseline_realistic"]) > 0 else None

        if v40_baseline and v43_baseline is not None:
            compare_metrics = ["return_pct", "cagr", "max_dd", "pf", "wr", "trades", "sharpe", "sortino", "fees_total", "funding_total"]
            for m in compare_metrics:
                v40_val = v40_baseline.get(m, 0)
                v43_val = v43_baseline.get(m, 0)
                delta = float(v43_val) - float(v40_val) if v40_val is not None and v43_val is not None else 0
                sign = "+" if delta > 0 else ""
                lines.append(f"| {m} | {v40_val} | {v43_val} | {sign}{round(delta, 2)} |")
        lines.append("")
...
def main() -> None:
    ...
    out_dir = repo_root / "docs/backtests/v43_hyperliquid_optimized"
    out_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 70)
    print("  v43 Hyperliquid BTC — Strict Validation")
    print("=" * 70)
    ...
    # ── build features ────────────────────────────────────────────────────
    print("[4/5] Building v43 features...")
    features = build_features_v43(strict_4h, strict_1d, funding)
    hourly_prices = strict_4h["close"].reindex(funding.index, method="ffill")

    # ── adx / funding filter coverage ─────────────────────────────────────
    ...
    for scenario in scenarios:
        ...
        trades, eq, cost_breakdown = StrictEngineV43(scenario, funding, hourly_prices).run(features)
    ...
    if stressed["return_pct"] <= 0 or stressed["pf"] < 1.0 or baseline["return_pct"] <= 0:
        verdict = "NO-GO"
        recommendation = (
            "The v43 edge does not survive conservative execution stress cleanly enough for live deployment. "
        ...
        verdict = "CONDITIONAL"
        recommendation = (
            "The v43 strategy survives realistic execution, but the edge is modest and it badly underperforms passive long BTC on the same window. "
        ...
        verdict = "GO"
        recommendation = (
            "The v43 strategy survives strict realism well enough to justify a tiny live pilot, "
            "but only with ongoing monitoring of slippage, stop execution, and funding drift."
        )

    write_report(out_dir, diagnostics, scenario_table[["scenario", "return_pct", "cagr", "max_dd", "pf", "wr", "trades", "sharpe", "sortino", "fees_total", "funding_total"]], buy_hold, v40_results, recommendation, verdict)
    ...
    # ── v40 comparison table ────────────────────────────────────────────────
    if v40_results:
        v40_scenarios = v40_results.get("scenarios", [])
        v40_baseline = next((r for r in v40_scenarios if r.get("scenario") == "baseline_realistic"), None)
        v40_stressed = next((r for r in v40_scenarios if r.get("scenario") == "stressed_conservative"), None)

        print("\n" + "=" * 70)
        print("  v40 vs v43 COMPARISON")
        print("=" * 70)
        if v40_baseline:
            print(f"\n  {'Metric':<20} {'v40 Baseline':>14} {'v43 Baseline':>14} {'Δ':>10}")
            print(f"  {'-'*58}")
            for m in ["return_pct", "cagr", "max_dd", "pf", "wr", "trades", "sharpe", "sortino", "fees_total", "funding_total"]:
                v40v = v40_baseline.get(m, 0)
                v43v = baseline.get(m, 0)
                d = float(v43v) - float(v40v) if v40v is not None and v43v is not None else 0
                print(f"  {m:<20} {str(v40v):>14} {str(v43v):>14} {d:>+10.2f}")

        if v40_stressed:
            print(f"\n  {'Metric':<20} {'v40 Stressed':>14} {'v43 Stressed':>14} {'Δ':>10}")
            print(f"  {'-'*58}")
            for m in ["return_pct", "cagr", "max_dd", "pf", "wr", "trades", "sharpe", "sortino", "fees_total", "funding_total"]:
                v40v = v40_stressed.get(m, 0)
                v43v = stressed.get(m, 0)
                d = float(v43v) - float(v40v) if v40v is not None and v43v is not None else 0
                print(f"  {m:<20} {str(v40v):>14} {str(v43v):>14} {d:>+10.2f}")
