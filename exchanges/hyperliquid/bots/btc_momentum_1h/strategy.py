...
class BTCMomentum1hStrategy:
    ...
    def _exit_signal(self, bar: Bar, position: Position) -> Signal:
        ...
        if position.side == "long":
            ...
            # Max hold bars
            if self._bar_count(position, bar.time) >= self.max_hold_bars:
                return Signal.CLOSE_LONG
    ...
    def _bar_count(self, position: Position, current_bar_time: pd.Timestamp) -> int:
        """Count 1h bars between entry and current bar timestamps."""
        ...
        bars = int((current_ts - entry_ts).total_seconds() / 3600)
        return max(bars, 0)
