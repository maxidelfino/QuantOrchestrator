...
export class PositionTracker {
  ...
  private async syncLoop(user: NordUser, accountId: number, marketId: number): Promise<void> {
  ...
  private async syncFromServer(user: NordUser, accountId: number, marketId: number): Promise<void> {
    try {
      await user.fetchInfo();
      const positions = user.positions[accountId] || [];
      const pos = positions.find((p) => p.marketId === marketId);
      const serverSize = pos?.perp
        ? pos.perp.isLong ? pos.perp.baseSize : -pos.perp.baseSize
        : 0;
      if (Math.abs(this.baseSize - serverSize) > 0.0001) {
        log.warn(`Position drift: local=${this.baseSize.toFixed(6)}, server=${serverSize.toFixed(6)}`);
  ...
  applyFill(side: "bid" | "ask", size: number, _price: number): void {
    ...
    log.debug(`Position updated: ${this.baseSize.toFixed(6)} (${side} ${size})`);
  }
  ...
  getQuotingContext(fairPrice: number): QuotingContext {
    ...
    return { fairPrice, positionState, allowedSides };
  }
  ...
  private getState(fairPrice: number): PositionState {
    ...
    return { sizeBase, sizeUsd, isLong, isCloseMode };
  }

  private getAllowedSides(state: PositionState): ("bid" | "ask")[] {
    if (state.isCloseMode) return state.isLong ? ["ask"] : ["bid"];
    return ["bid", "ask"];
  }
  ...
  isCloseMode(fairPrice: number): boolean {
    return Math.abs(this.baseSize * fairPrice) >= this.config.closeThresholdUsd;
  }
}
